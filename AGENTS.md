...
* Every mutation must validate first and commit the complete result, undo history, and pending export revision to SQLite transactionally. Web saves then queue coalesced background exports and return the persisted content with an explicit export status; the UI must distinguish saved content from pending or failed exports. Normal server shutdown drains accepted export jobs, and startup repairs pending/failed exports from SQLite. CLI operations and explicit export requests remain synchronous. A failed database transaction must not publish exports.
...
* Every destructive action must also appear in the global recovery center with an explicit deletion type. The recycle bin must distinguish plots, characters, entries, fragments, relationships, timeline lines, and chapters; restoring a structural deletion must safely undo the complete persisted operation. Do not recreate a diagnostics/checks tab unless the user explicitly asks for one.
...
* Ordinary background exports update affected Markdown and bounded static/recovery journals. Preserve immutable `export-data/` dependencies when moving a journal package; explicit exports remain full, and static packaging must materialize a standalone reading snapshot. Keep normal saves free of unrelated full-body and attachment reads.
