...
图谱不继续在“Canvas 绘制连线 + DOM 绘制节点”的混合方案上堆叠性能补丁，也不把切换到 WebGPU 当作首要解决方案。此前混合渲染曾经出现过节点移动而连线留在原处、短暂闪烁以及缩放后坐标不一致等问题。即使当前实现通过共享 `animatedPoints` 暂时保证了两层使用同一份位置，双渲染层仍然会增加浏览器合成、缓存失效和交互同步的复杂度。

目标架构为“单一场景状态 + 单一图谱主体渲染器”：
...
图谱场景状态
  ...
  └── 选中、聚焦和可见性
          ↓
布局层：只在数据、尺寸或手动位置改变时计算坐标
          ↓
统一渲染器：在同一渲染层绘制节点、连线、标签和选中状态
          ↓
交互适配：把指针坐标映射回同一份场景状态
```

具体约束：

- 节点、连线和标签必须从同一份场景坐标绘制，禁止为 DOM 和 Canvas 分别维护位置；
- React 只负责传入场景数据、保存用户修改和显示详情面板，不负责逐节点驱动图谱主体渲染；
- 图谱主体默认静态绘制；拖拽、缩放、聚焦时才进入高频重绘；
- 粒子、呼吸、漂浮等装饰动画不属于基础交互，默认关闭或仅对当前选中关系启用；
- 详情面板、编辑器和键盘辅助可以继续使用 DOM，但不能成为图谱坐标或连线状态的第二个事实来源；
- 组织区域默认以聚合/分组信息表达，不为组织成员关系无限增加常驻连线；
- 单向关系显示一条线，双向关系显示两条带方向的线；关系标签只在有意义或被聚焦时显示，避免文字成为默认渲染负担。

迁移期间允许保留现有 Canvas + DOM 实现，但必须把 `animatedPoints` 提升为明确的场景坐标模型，并先补齐同步回归测试。迁移完成后再评估静态图层缓存、Canvas 分层或 WebGPU；任何优化都不得重新引入两套坐标源。

图谱渲染的最低验收标准：

- 连续拖拽节点时，任意一帧的连线端点都与节点中心重合；
- 拖拽结束、取消拖拽、缩放、平移和窗口调整后，节点与连线没有残影或偏移；
- 单线、双线、关系标签、印象标签和组织聚合在相同 viewport 下保持一致；
- 没有交互或动态效果时，不持续运行高频动画循环；
- 隐藏关系、聚焦关系和选中状态只改变渲染结果，不改变底层实体关系；
- 在支持与不支持 WebGPU 的浏览器中，场景坐标、命中测试和交互语义保持一致。
