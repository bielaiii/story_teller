...
export default function FragmentsPage() {
  ...
  return <section className={`workspace-page fragments-page-new${fragmentView === "board" ? " is-board-view" : ""}`}>
    <header className="page-header"><div><small>Idea Inbox</small><h1>灵感碎片箱</h1><p>{fragmentView === "board" ? "像知识图谱一样查看碎片：连接来自剧情顺序、明确引用、共同人物和标签。" : "单章直接阅读，剧情线在悬浮窗口中按章节继续推演。"}</p></div><div className="fragment-page-header-actions"><div className="fragment-view-switch" role="group" aria-label="碎片显示方式"><button type="button" className={fragmentView === "cards" ? "is-active" : ""} aria-pressed={fragmentView === "cards"} onClick={() => setFragmentView("cards")}><Icon name="book" />卡片</button><button type="button" className={fragmentView === "board" ? "is-active" : ""} aria-pressed={fragmentView === "board"} onClick={() => setFragmentView("board")}><Icon name="timeline" />关系图</button></div>{writable && <div className="fragment-page-actions">{supportsClipboardImport && <button className="fragment-import-action" disabled={mutation.isPending} onClick={importFromClipboard}><span><Icon name="clipboard" /></span>{mutation.isPending ? "正在解析…" : "从剪贴板导入"}</button>}<button className="fragment-create-action" onClick={() => openNew()}><span><Icon name="plus" /></span>新建碎片</button></div>}</div></header>
    ...
    {fragmentView === "board" ? snapshot.fragments.length > 0 && <FragmentBoard fragments={snapshot.fragments} selectedTags={selectedTags} allTags={tags} onEdit={openEditor} onImmersive={setReader} /> : <><div className="fragment-grid-new">{fragments.map((item, index) => {
      ...
      return <ReactFragment key={item.entityId}>
        ...
        {expanded && <div className="dialog-backdrop fragment-line-floating-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && toggle()}><section className="fragment-line-expanded" role="dialog" aria-modal="true" aria-label={`${item.title}的章节`} onMouseDown={(event) => event.stopPropagation()}>
