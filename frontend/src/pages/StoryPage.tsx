    ...
    <div className="dialog-backdrop editor-backdrop" role="presentation">
      <section className="editor-dialog" role="dialog" aria-modal="true" aria-label={currentId === "new" ? "写新剧情" : `编辑${currentChapterTitle}`}>
        ...
        <EditorSettingsSection label="剧情设置">
          ...
          <label className="wide"><span>故事</span><select aria-label="故事" value={draft.stories.find((storyId) => storyId !== snapshot.timeline.mainLineId) || ""} onChange={(event) => change("stories", event.target.value ? [event.target.value] : [])}><option value="">主线（默认）</option>{snapshot.timeline.lines.filter((item) => item.entityId !== snapshot.timeline.mainLineId).map((item) => <option key={item.entityId} value={item.entityId}>{item.name}</option>)}</select><small>选择这段剧情所属的故事线</small></label>
