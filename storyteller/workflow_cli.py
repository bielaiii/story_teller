...
def register_workflow_domains(domains: argparse._SubParsersAction) -> None:
    ...
    line_list = line_commands.add_parser("list", help="列出全部剧情线")
    line_list.set_defaults(handler=timeline_line_list_command)
    line_add = line_commands.add_parser("add", help="新增剧情线")
    ...
    line_edit = line_commands.add_parser("edit", help="编辑剧情线名称、颜色或显示侧")
    ...
    line_delete = line_commands.add_parser("delete", help="删除剧情线并转移其节点")
    ...
    node_list = node_commands.add_parser("list", help="列出剧情节点及所属剧情线")
    ...
    history = timeline_commands.add_parser("history", help="列出时间线操作历史")
    history.add_argument("--limit", type=int, default=100)
    history.set_defaults(handler=timeline_history_command)
    undo = timeline_commands.add_parser("undo", help="撤销一次时间线操作")
